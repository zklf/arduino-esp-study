...
unsigned long startTime; // 记录程序开始运行的时间

unsigned long lastScreenRefreshTime; // 记录上一次屏幕刷新的时间
const unsigned int screenRefreshInterval = 5000; // 屏幕刷新间隔，单位为毫秒
 ...
 //检查是否断开，断开后重连
  ...
  else
  {
    ...
    if(millis() - preHeartTick >= upDataTime){//上传数据
      ...
      String pressure_bmp_str = String(pressure_bmp, 4); // 保留2位小数
...
void displayData(byte temperature, byte humidity, float temperature_bmp, float pressure_bmp) {
  ...
  u8g2.setCursor(0, 12);
  u8g2.print("Uptime: ");
  u8g2.print(elapsedTime / 1000); // 将总时间转换为秒并打印到屏幕上
  u8g2.print(" s");
